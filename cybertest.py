...
def track_pixel(email_id):
    log_line = f"[{datetime.now()}] EMAIL OPENED: {email_id}  - UA: {request.headers.get('User-Agent')}\n"
...
def click_tracker():
    ...
    log_line = f"[{datetime.now()}] LINK CLICKED: {user} - UA: {ua}\n"
    with open("click_log.txt", "a") as f:
        f.write(log_line)

    return redirect("https://alwaysjudgeabookbyitscover.com/guides/coloring-books/", code=302)
